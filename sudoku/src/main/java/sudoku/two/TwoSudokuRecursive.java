...
public class TwoSudokuRecursive {
	private final int SIZE = 2;
	private int[][] board;
	public int nodes = 0;
	public int[][] output = new int[2][2];
	public int solutions = 0;

	public TwoSudokuRecursive(int[][] board) {
		this.board = board;
	}
	...
	boolean nonConflict(int row, int column, int candidate) {
		...
		for (int iRow = 0; iRow < SIZE; iRow++)
			if (board[iRow][column] == candidate)
				return false;
		int lr = row / 1, cr = column / 1;
		for (int iRow = lr * 1; iRow < (lr + 1) * 1; iRow++)
			for (int iColunm = cr * 1; iColunm < (cr + 1) * 1; iColunm++)
